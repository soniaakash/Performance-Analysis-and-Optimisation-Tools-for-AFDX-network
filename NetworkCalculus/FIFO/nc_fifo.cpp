...
void NC_FIFO::computeE2E(NCData *dataPtr){
    // prerequisite
    if(dataPtr == nullptr) throw std::runtime_error("Invalid NCData in :: NC_FIFO::computeE2E()");
...
double NC_FIFO::computeNodeDelay(const NCData::NCFlow &refFlow, const NODE_t &node)
{

    // prerequisite
    if(ncData == nullptr) throw std::runtime_error("Invalid NCData in :: NC_FIFO::computeNodeDelay()");
...
//  Step 3a: compute overall arrival curve (alpha_o)
ConcaveCurve NC_FIFO::computeOverallArrivalCurve(const NCData::NCFlow &refFlow, const NODE_t &node)
{
    // prerequisite
    if(ncData == nullptr) throw std::runtime_error("Invalid NCData in :: NC_FIFO::computeOverallArrivalCurve()");;
...
double NC_FIFO::computeJitter(const NCData::NCFlow &refFlow, const NODE_t &node)
{
    // prerequisite
    if(ncData == nullptr) throw std::runtime_error("Invalid NCData in :: NC_FIFO::computeJitter()");;
    // NCData::NCNode &ncNode = ncData->selectNCNode_ref(node.first);
...
//  Step 3b: compute service curve (beta_Vref)
ConvexCurve NC_FIFO::computeServiceCurve(const NCData::NCFlow &refFlow, const NODE_t &node)
{
    // prerequisite
    if(ncData == nullptr) throw std::runtime_error("Invalid NCData in :: NC_FIFO::computeServiceCurve()");;
    NCData::NCNode &ncNode = ncData->selectNCNode_ref(node.first);
    // QList<NCData::NCNode>::iterator ncNode = ncData->selectNCNode(node.first);
